...
const AddLiquidityABI = [
    createAbiItem('addLiquidity', ['address', 'uint256', 'uint256'], [''])
]
...
class AddLiquidityContract extends Web3Contract {
    constructor(address) {
        super(AddLiquidityABI, address)
    }

    addLiquidity(bondAsset, bondDuration, bondAmount, gasPrice) {
        if (!this.account) {
            return Promise.reject()
        }

        return this.send('addLiquidity', [bondAsset, bondDuration, bondAmount], {
            from: this.account,
...
export default AddLiquidityContract